...
public class Methods {
    ...
    public String getSplitStringBuilder(String value, String splitValue, String mapKeySuffix) {
        ...
        for (String text : values) {
            if (text.endsWith(mapKeySuffix)) {
                text = BaseTest.TestMap.get(text).toString();
            }
    ...
    public void setStepJsonObjectValue(JsonObject jsonObject, String property, String value, String valueType) {
        ...
        if (!(valueType.equals("JsonArray") || valueType.equals("JsonObject") || valueType.equals("JsonElement"))
            ...
            if (BaseTest.TestMap.containsKey(value)) {
        ...
        switch (valueType) {
            case "":
                break;
            case "JsonArray":
                jsonObject.add(property, (JsonArray) BaseTest.TestMap.get(value));
                break;
            case "JsonObject":
                jsonObject.add(property, (JsonObject) BaseTest.TestMap.get(value));
                break;
            case "JsonElement":
                jsonObject.add(property, (JsonElement) BaseTest.TestMap.get(value));
                break;
            case "String":
    ...
    public void setStepJsonArrayValue(JsonArray jsonArray, String value, String valueType) {
        ...
        switch (valueType) {
            case "":
                break;
            case "JsonArray":
                jsonArray.add((JsonArray) BaseTest.TestMap.get(value));
                break;
            case "JsonObject":
                jsonArray.add((JsonObject) BaseTest.TestMap.get(value));
                break;
            case "JsonElement":
                jsonArray.add((JsonElement) BaseTest.TestMap.get(value));
                break;
            case "String":
                jsonArray.add(value);
                break;
    ...
    public String setStringJson(String value, String splitValue, String newValues, String types) {
        ...
        for (int i = 0; i < arrayValue.length; i++) {
            a = arrayValue[i];
            a = a.endsWith("KeyValue") ? BaseTest.TestMap.get(a).toString() : a;
            switch (arrayTypes[i]) {
                case 'S':
    ...
    public String setValueWithMap(String value) {
        ...
        while (matcher3.find()) {
            ...
            value = value.replace(t, BaseTest.TestMap
    ...
    public String setJsonWithMapKey(String value) {
        ...
        while (matcher3.find()) {
            ...
            value = value.replace(t, BaseTest.TestMap.get(mapValue).toString());
        }
        return value;
    }
    ...
    public String getJsonValueWithMap(String mapKey) {
        ...
        if (BaseTest.apiMap.containsKey(mapKey)) {
            jsonString = ((Response) BaseTest.apiMap.get(mapKey).get("response")).asString();
        }
        if (mapKey.endsWith("KeyValue")) {
            jsonString = BaseTest.TestMap.get(mapKey).toString();
            JsonElement element = JsonParser.parseString(jsonString);
    ...
    public String getTextByMap(String text) {
        ...
        return text.endsWith("KeyValue") ? BaseTest.TestMap.get(text).toString() : text;
    }

    public ResourceBundle readPropDir(String dir) {
